...
import { LayerController } from "pkg/cesium/layer-controller";
import {
  LegendsType,
  type BaseLayerOptions,
  type LegendsOptions
} from "pkg/cesium/types/layer";
import { getLayerConfig } from "@/api/map";
...
const legendStyle = computed<CSSProperties>(() => {
  ...
  if (legendsOptions.value) {
    legendsOptions.value.position.left &&
      (style.left = legendsOptions.value.position.left + "px");
    legendsOptions.value.position.right &&
      (style.right = legendsOptions.value.position.right + "px");
    legendsOptions.value.position.top &&
      (style.top = legendsOptions.value.position.top + "px");
